...
- [x] **3주차: 주문 도메인 + 도메인 이벤트 + API 유효성 검증**
  - [x] 주문 도메인 구현 (주문 Aggregate, 주문항목 VO, 주문상태 전이 다이어그램 작성)
  - [x] Spring ApplicationEvent 기반 내부 이벤트 발행 (`OrderPlacedEvent`, `StockDeductedEvent`)
  - [x] 트랜잭션 경계 설계 (주문 생성 → 재고 차감 → 결제 요청 흐름의 보상 트랜잭션 설계)
  - [x] 낙관적 락(Optimistic Lock)을 이용한 재고 동시성 제어 적용
  - [x] API 유효성 검증(Bean Validation) 도입 (기존 상품/장바구니 DTO 보완 및 주문 DTO 검증 적용)
  - [x] *산출물*: 주문 API, 이벤트 흐름도, 상태 전이 다이어그램, DTO 유효성 검증 규칙


### 2단계 (4~6주): 결제 + 정산 배치
외부 연동 및 트랜잭션 안전성 확보, 대용량 정산 처리를 목표로 합니다.
- [x] **4주차: PG 결제 연동 + 결제 상태 머신**
  - [x] 포트원(구 아임포트) 테스트 계정 생성 및 API 연동 흐름 파악
  - [x] 결제 상태 머신 설계 (`INITIATED` → `PG_REQUESTED` → `APPROVED` → `COMPLETED` / `FAILED` / `CANCELLED`)
  - [x] 중복 결제 방지를 위한 멱등성 키(Idempotency Key) 설계 및 적용
  - [x] 결제 완료 콜백 검증 로직 구현 (PG 서버 측 금액 및 상품 정보 재검증)
  - [x] *산출물*: 결제 상태 전이 다이어그램, 결제 도메인 API
- [ ] **5주차: 결제 보안 + 데이터 보호**
